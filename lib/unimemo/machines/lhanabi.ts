...
export const lhanabiUnimemoConfig: UnimemoMachineConfig = {
  ...
  layoutGuide: `
- 基本情報: 総プレイ数 -> 花火チャレンジプレイ数 -> 花火GAMEプレイ数 -> 総ボーナス回数 -> 総BB回数 -> 総ドンBB回数 -> 総赤七BB回数 -> 総RB回数
- 通常中小役: 風鈴A回数 -> 風鈴B回数 -> 風鈴合算 -> 氷A回数 -> 氷B回数 -> 氷合算 -> チェリーA1回数 -> チェリーA2回数 -> チェリーB回数 -> チェリー合算
- RT中小役: 花火チャレンジはずれ回数 -> 花火GAMEはずれ回数
- BB中小役: 風鈴A回数 -> 風鈴B回数 -> ハズレ目またはバラケ目
- RB中小役: 1枚役回数 -> ハズレ目またはバラケ目
- BGM: 大花火BGM回数 -> ドンちゃん2BGM回数
- その他: 最大獲得枚数 -> 最大連RT連チャン回数 -> 最大数珠連回数 -> RB終了画面ピース表示回数
- リーチ目スコア: 獲得スコア
- リーチ目コレクション: 新規
`,
  fields: [
    ...
    { key: "regPieceLowHits", kind: "integer", description: "その他のRB終了画面ピース表示回数。例: 3。見当たらない場合はnull" }
  ],
  prompt: `
スマスロ Lハナビのユニメモ遊技履歴画像から、推測入力に必要な数値だけを読み取ってください。

読み取り対象:
- 基本情報: 総プレイ数、総BB回数、総RB回数
...
「チェリー合算」「風鈴合算」「氷」「BGM」「リーチ目スコア」「最大獲得枚数」「最大連RT連チャン回数」「最大数珠連回数」は使わないでください。
